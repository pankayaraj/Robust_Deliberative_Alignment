...
import csv
def save_prompt_response_csv(list_a, list_b, list_c, list_d, list_e, list_f, filename, col1="prompt", col2="response", col3="label", col4="perplexity", col5="self_certainity",  col6="gini"):
...
def compute_self_certaininty(inputs, model, prompt_length):
    ...
    adjusted_log_probs = log_probs + torch.log(torch.tensor(vocab_size))
    per_token_sc = -adjusted_log_probs.mean(dim=-1)
    ...
    masked_sc = per_token_sc * mask

    sum_sc = masked_sc.sum(dim=1)
    count_tokens = mask.sum(dim=1)
    avg_loss = sum_sc / count_tokens
...
def compute_gini_impurity(inputs, model, prompt_length):
    ...
    sum_squares = torch.sum(probs**2, dim=-1)
    gini = 1.0 - sum_squares

    attention_mask = inputs["attention_mask"]
    attention_mask[:,:prompt_length] = torch.zeros(inputs["attention_mask"].shape[0], prompt_length)
    mask = attention_mask[:, :].contiguous()
    masked_gini = gini * mask

    sum_gini = masked_gini.sum(dim=1)
    count_tokens = mask.sum(dim=1)
    avg_gini = sum_gini / count_tokens
    
    result = avg_gini.cpu().tolist()
    if type(result) == float:
        result = [result]
    return result
...
def compute_perplexity(inputs, model, prompt_length):
    ...
    per_sample_perplexity = torch.exp(avg_loss).squeeze()

    result = per_sample_perplexity.cpu().tolist()
    if type(result) == float:
        result = [result]
    return result
...
if DATASETS[0] == 1:
    ...
    for idx in tqdm(range(0, len(PROMPTS))):
        ...
        full_ids = tokenizer(full_pairs, return_tensors="pt", max_length=2048, padding=True, truncation=True, padding_side="right")
        ...
        preplexity = compute_perplexity(full_ids, model, prompt_length)
        preplexity = [str(p) for p in preplexity]
        PREPLEXITY.append(preplexity)

        self_certainity = compute_self_certaininty(full_ids, model, prompt_length)
        self_certainity = [str(s) for s in self_certainity]
        SELF_CERTAINITY.append(self_certainity)
        
        gini_impurity = compute_gini_impurity(full_ids, model, prompt_length)
        gini_impurity = [str(s) for s in gini_impurity]
        GINI_IMPURITY.append(gini_impurity)
    ...
    save_prompt_response_csv(
        ...
        SELF_CERTAINITY,
        GINI_IMPURITY,
        args.dir + "/mulit_generation_certainity_wildjailbreak.csv"
    )
...
if DATASETS[1] == 1:
    ...
    save_prompt_response_csv(
        ...
        SELF_CERTAINITY,
        GINI_IMPURITY,
        args.dir + "/mulit_generation_certainity_strongreject.csv"
    )
...
if DATASETS[2] == 1:
    ...
    save_prompt_response_csv(
        ...
        SELF_CERTAINITY,
        GINI_IMPURITY,
        args.dir + "/mulit_generation_certainity_DAN.csv"
    )
